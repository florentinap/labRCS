...
def get_transition_probabilities(grid):
    ...
    for i in range(N):
        for j in range(N):
            ...
            neighbours = grid.get_neighbours(si)
            elevation = [grid.elevation[e] for e in [n[0] for n in neighbours]]
            max_e = max(elevation)
            Nmax = len([e for e in elevation if e == max_e])
            Nother = len(neighbours)

            if sj in [n[0] for n in neighbours]:
                if grid.elevation[sj[0]][sj[1]] == max_e:
                    A[i][j] = grid.p_t/Nmax + (1 - grid.p_t)/Nother
                else:
                    A[i][j] = (1 - grid.p_t)/Nother
    ...
    for t in range(T):
        for s in range(N):
            if t == 0:
                ...
                for ss in range(N):
                    alpha[t][s] += alpha[t-1][ss] * A[ss][s] * B[s][observations[t]]
    # TODO 3 ends here
    ...
    for t in range(T):
        for s in range(N):
            if t == 0:
                ...
                delta[t][s] = max([delta[t - 1][si] * A[si][s] * B[s][observations[t]] for si in range(N)])
        maxDelta = max(delta[t - 1][ss] for ss in range(N))
        for ss in range(N): 
            if delta[t-1][ss] == maxDelta:
                states[t] = ss
                break
    # Task 4 ends here.
